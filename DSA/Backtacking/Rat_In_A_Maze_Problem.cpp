...
In the given binary matrix, 0 signifies the cell is a dead end and 1 signifies the cell can be used to move from source to destination
 cell.
...
Each of the following N lines denote rows of the binary matrix and contains either 0 or 1. Each row of the binary matrix contains N 
elements. 
Output Format :
...
and separated by space. The cells that are part of the path should be 1 and remaining all cells should be 0. Please refer to sample 
...
void ratSolution(int row, int col, int n) {
  ...
  if (check[row][col] == 1 || row >= n || col >= n || row < 0 || col < 0 ||
      maze[row][col] == 0) {
    return;
  }
  check[row][col] = 1;
