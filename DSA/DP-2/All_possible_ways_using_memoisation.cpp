/*
All possible ways
Given two integers a and b. You need to find and return the count of possible ways in which we can represent the number a as the sum of
...
int getAllWaysHelper(int a, int b, int base, int *output)
{
    if (a < pow(base, b))
        return 0;
    if (a == pow(base, b))
    ...
    output[a] = getAllWaysHelper(a - pow(base, b), b, base + 1, output) + getAllWaysHelper(a, b, base + 1, output);
...
int getAllWays(int a, int b) {
 int *dp = new int [10000];
  ...
  for (int i = 0; i <= a+1; i++) {
    ...
    return getAllWaysHelper(a,b,1,dp);
}
int main()
{
    int a, b;
